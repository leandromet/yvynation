...
class AppState(
    ...
    def idle_guard_busy(self) -> bool:
        ...
        """
        return (
            # `batch_busy`, not `batch_running`: a dispatched run does not set
            # `batch_running` until it has snapshotted its configuration and
            # cleared the abuse-control checks, several seconds later (see
            # state/_batch.py::batch_starting). Pausing the session in that
            # window would disconnect the WebSocket from a run that is on its
            # way — the exact thing this guard exists to prevent.
            self.batch_busy
