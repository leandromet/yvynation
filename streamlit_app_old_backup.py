...
def create_ee_folium_map(center, zoom, layer1_year, layer1_opacity=1.0, 
    ...
    try:
        ...
        print(f"layer1_year: {layer1_year}")
        print(f"drawn_polygon: {drawn_polygon}")
        ...
        print(f"✓ Data loaded, creating map...")
        ...
        if data_source == "MapBiomas":
            print(f"Loading MapBiomas layer...")
            # MapBiomas layers
            ...
            print(f"✓ MapBiomas loaded, layer1_year: {layer1_year}")
            # Layer 1 (default: 2023)
            if isinstance(layer1_year, int):
                layer1_band = f'classification_{layer1_year}'
                print(f"  Selecting band: {layer1_band}")
                layer1_image = mapbiomas.select(layer1_band)
                
                # Get tile URL from EE image using getMapId
                print(f"  Getting map ID from EE...")
                vis_params = {'min': 0, 'max': 62, 'palette': MAPBIOMAS_PALETTE}
                try:
                    map_id = layer1_image.getMapId(vis_params)
                    print(f"  ✓ Map ID obtained: {map_id['tile_fetcher'].url_format[:100]}")
                    folium.TileLayer(
                    ...
                    print(f"✓ Layer 1 added to map object, map has {len(m._children)} children")
                except Exception as e:
                    print(f"❌ Error adding MapBiomas layer: {e}")
                    import traceback
                    traceback.print_exc()
                    st.warning(f"Could not add MapBiomas layer: {e}")
            
            # Layer 2 (comparison mode, default: 1985)
            if compare_mode and layer2_year:
                layer2_band = f'classification_{layer2_year}'
                print(f"  Selecting band: {layer2_band}")
                layer2_image = mapbiomas.select(layer2_band)
                
                print(f"  Adding MapBiomas comparison layer to map...")
                vis_params2 = {'min': 0, 'max': 62, 'palette': MAPBIOMAS_PALETTE}
                try:
                    map_id2 = layer2_image.getMapId(vis_params2)
                    folium.TileLayer(
                    ...
                    print(f"✓ Layer 2 added")
                except Exception as e:
                    print(f"❌ Error adding MapBiomas layer 2: {e}")
            
            # Add indigenous territories layer on top (dark purple) with interactive popups
            print(f"Adding territories layer...")
            territories = st.session_state.app.territories
            if territories is not None:
                try:
                    print(f"  Adding territories image...")
                    territories_image = ee.Image().paint(territories, 1, 2)
                    vis_params_terr = {'min': 0, 'max': 1, 'palette': ['00000000', '4B0082']}
                    map_id_terr = territories_image.getMapId(vis_params_terr)
                    folium.TileLayer(
                    ...
                    print(f"  Adding territory popups...")
                    m = add_territory_popups_to_map(m, territories, name_prop='NAME')
                    print(f"✓ Territories layer added")
                except Exception as e:
            ...
            print(f"  year_key: {year_key}")
            
            # Apply ocean mask to the Hansen image
            print(f"  Loading ocean mask...")
            landmask = ee.Image(HANSEN_OCEAN_MASK).lte(1)
            print(f"  Loading Hansen dataset...")
            ...
            print(f"  Adding Hansen layer to map...")
            try:
                ...
                print(f"✓ Hansen layer added")
            except Exception as e:
                print(f"❌ Error adding Hansen layer: {e}")
        ...
        if drawn_polygon:
            try:
                ...
                # If it's a dict with 'coordinates' key (GeoJSON format)
                if isinstance(drawn_polygon, dict):
                    if 'coordinates' in drawn_polygon:
                        coords_to_draw = drawn_polygon['coordinates']
                    else:
                        st.warning(f"Polygon dict missing 'coordinates' key")
                        coords_to_draw = None
                else:
                ...
                st.warning(f"Could not draw polygon: {type(e).__name__}: {e}")
            except Exception as e:
                st.warning(f"Unexpected error drawing polygon: {e}")
        
        # Add drawing tools
        # Note: Draw layer can cause issues with st_folium bounds calculation
        # st_folium provides its own drawing capabilities via the Draw=True parameter
        ...
        import traceback
        error_msg = f"Map creation error: {str(e)}\n\n{traceback.format_exc()}"
        st.error(error_msg)
        return None
    ...
    with map_col:
        ...
        if st.session_state.data_loaded:
            try:
                ...
                print(f"Display map section - map object exists: {st.session_state.mapbiomas_map_object is not None}")
                if st.session_state.mapbiomas_map_object:
                    print(f"  Displaying map with st_folium...")
                    try:
                        map_data = st_folium(st.session_state.mapbiomas_map_object, width="100%", height=600)
                        print(f"  ✓ st_folium returned, got map_data: {map_data is not None}")
                    except KeyError as ke:
                        print(f"  ⚠️ st_folium bounds error: {ke}, using fallback display...")
                        # Fallback: display the map without interaction if bounds calculation fails
                        from streamlit_folium import folium_static
                        folium_static(st.session_state.mapbiomas_map_object)
                        map_data = None
                ...
                print(f"❌ EXCEPTION in map display: {type(e).__name__}: {str(e)}")
                import traceback
                traceback.print_exc()
    ...
    with map_col_h:
        ...
        if st.session_state.data_loaded:
            try:
                ...
                # Display map and capture drawn areas
                print(f"Display map section - map object exists: {st.session_state.hansen_map_object is not None}")
                if st.session_state.hansen_map_object:
                    ...
                    try:
                        ...
                        # Fallback: display the map without interaction if bounds calculation fails
                        from streamlit_folium import folium_static
                        folium_static(st.session_state.hansen_map_object)
                        map_data = None
