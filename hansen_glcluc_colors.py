"""
Hansen/GLAD GLCLUC Color Palette
Based on legend_glcluc.csv - the official color scheme for Hansen classes
"""

# The 256-color Hansen/GLAD palette from legend_glcluc.csv
HANSEN_GLCLUC_PALETTE = [
...
# Class ID to color mapping (0-255)
HANSEN_CLASS_COLORS = {}
for class_id, color in enumerate(HANSEN_GLCLUC_PALETTE):
    HANSEN_CLASS_COLORS[class_id] = f"#{color}"

def get_hansen_class_color(class_id):
    """Get the official GLCLUC color for a Hansen class ID"""
    if isinstance(class_id, (int, float)):
        class_id = int(class_id)
    return HANSEN_CLASS_COLORS.get(class_id, "#808080")
